...
function v3MergeProductArray(incoming,{showInfo=true,goProducts=false}={}){
  ...
  toast('製品を追加登録しました');
  if(showInfo){
    v3OpenInfo('製品リストの登録完了','<div class="info-stats"><p><span>新規追加</span><strong>'+added+'件</strong></p><p><span>既存を更新</span><strong>'+updated+'件</strong></p><p><span>読み飛ばし</span><strong>'+skipped+'件</strong></p></div><p class="info-note">既存の写真と作成日時は保持しています。同名製品は重複させず更新しました。</p>');
  }
...
async function v3CopyMonitorFeedback(){
  const template=[
    '【美容管理 Monitor v1｜不具合報告】',
    '発生した画面：'+(document.querySelector('.screen.active')?.dataset.screen||'不明'),
    '操作した内容：',
    '起きた症状：',
    '再現するか：毎回 / ときどき / 1回だけ',
    '製品件数：'+data.products.length,
    '施術件数：'+data.treatments.length,
    '履歴件数：'+data.history.length,
    '端末・ブラウザ：'+navigator.userAgent
  ].join('\n');
  try{
    await navigator.clipboard.writeText(template);
    toast('不具合報告テンプレートをコピーしました');
  }catch{
    const ta=document.createElement('textarea');
    ta.value=template;ta.style.position='fixed';ta.style.opacity='0';
    document.body.appendChild(ta);ta.select();
    let ok=false;
    try{ok=document.execCommand('copy')}catch{}
    ta.remove();
    if(ok)toast('不具合報告テンプレートをコピーしました');
    else v3OpenInfo('不具合報告テンプレート','<pre class="feedback-template">'+v2Esc(template)+'</pre>');
  }
}
$('#monitorFeedbackBtn')?.addEventListener('click',v3CopyMonitorFeedback);
