(() => {
  ...
  function renderProducts(){
    ...
    return `${appHeader("製品","写真からも登録できます")}<section class="card"><div class="row"><input id="product-search" class="grow" placeholder="製品名・ブランドで検索" value="${esc(productQuery)}"><button class="btn" data-add-product>＋追加</button></div></section><section class="card">${list.length?list.map(p=>`<div class="list-row">${p.imageData?`<img class="product-thumb" src="${p.imageData}" alt="${esc(p.name)}の写真">`:`<div class="product-thumb placeholder">写真なし</div>`}<div class="item-main"><div class="item-name">${esc(p.name)}</div><div class="item-meta">${esc(p.brand||"ブランド未設定")} ・ ${esc(p.category)} ・ ${p.timing==="both"?"朝・夜":timeLabel(p.timing)}${p.opened?`<br>開封日: ${esc(p.opened)}`:""}${p.note?`<br>${esc(p.note)}`:""}</div></div><div class="list-actions"><button class="btn small secondary" data-edit-product="${p.id}">編集</button><button class="btn small danger" data-del-product="${p.id}">削除</button></div></div>`).join(""):`<div class="empty">製品がありません。<br>「＋追加」から写真付きで登録できます。</div>`}</section>`;
  }
  ...
  function openProductModal(id=""){
    const p=state.products.find(x=>x.id===id)||{name:"",brand:"",category:"美容液",timing:"both",opened:"",note:"",imageData:""};
    showModal(`<h3>${id?"製品を編集":"製品を追加"}</h3><form id="product-form"><div class="field"><label>製品写真</label><div class="product-photo-picker"><div id="product-photo-preview" class="product-photo-preview">${p.imageData?`<img src="${p.imageData}" alt="製品写真プレビュー">`:`<span>写真なし</span>`}</div><div class="row"><label class="btn secondary photo-label">📷 写真から登録<input id="product-photo-input" type="file" name="photo" accept="image/*" capture="environment" hidden></label>${p.imageData?`<button type="button" class="btn danger" data-remove-product-photo>写真を削除</button>`:""}</div><div class="sub">カメラ撮影または写真ライブラリから選べます。画像は端末内に圧縮保存されます。</div></div></div><div class="field"><label>製品名</label><input name="name" required value="${esc(p.name)}" placeholder="例：美容液A"></div><div class="field"><label>ブランド</label><input name="brand" value="${esc(p.brand)}"></div><div class="grid2"><div class="field"><label>カテゴリ</label><select name="category">${CATEGORIES.map(c=>`<option ${c===p.category?"selected":""}>${c}</option>`).join("")}</select></div><div class="field"><label>使用タイミング</label><select name="timing"><option value="morning" ${p.timing==="morning"?"selected":""}>朝</option><option value="night" ${p.timing==="night"?"selected":""}>夜</option><option value="both" ${p.timing==="both"?"selected":""}>朝・夜</option></select></div></div><div class="field"><label>開封日</label><input type="date" name="opened" value="${esc(p.opened||"")}"></div><div class="field"><label>メモ</label><textarea name="note">${esc(p.note||"")}</textarea></div><div class="row"><button type="button" class="btn ghost" data-close>キャンセル</button><button class="btn grow" type="submit">保存</button></div></form>`);
    let nextImageData=p.imageData||"";
    $("[data-close]").onclick=closeModal;
    $("#product-photo-input")?.addEventListener("change",async e=>{
      const file=e.target.files?.[0]; if(!file)return;
      try{
        nextImageData=await compressImage(file,640,.72);
        const preview=$("#product-photo-preview"); if(preview)preview.innerHTML=`<img src="${nextImageData}" alt="製品写真プレビュー">`;
      }catch{ alert("写真を読み込めませんでした"); }
    });
    $("[data-remove-product-photo]")?.addEventListener("click",()=>{
      nextImageData=""; const preview=$("#product-photo-preview"); if(preview)preview.innerHTML="<span>写真なし</span>";
    });
    $("#product-form").onsubmit=e=>{
      e.preventDefault();
      const fd=new FormData(e.currentTarget),data={id:id||uid(),name:String(fd.get("name")||"").trim(),brand:String(fd.get("brand")||"").trim(),category:String(fd.get("category")||"その他"),timing:String(fd.get("timing")||"both"),opened:String(fd.get("opened")||""),note:String(fd.get("note")||"").trim(),imageData:nextImageData};
      if(!data.name)return;
      const i=state.products.findIndex(x=>x.id===id); if(i>=0)state.products[i]=data; else state.products.push(data);
      try{saveState();}catch{alert("保存容量が足りません。製品写真を減らすか、不要な製品を削除してください。");return;}
      closeModal();render();
    };
  }
